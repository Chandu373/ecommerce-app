package com.chandu.notification.email;

import com.chandu.notification.enums.EmailTemplates;
import com.chandu.notification.record.Product;
...
public class EmailService {
    ...
    public  void sendPaymentSuccessEmail(String destinationEmail, String customerName, BigDecimal amount, String orderReference) throws MessagingException {
        ...
        messageHelper.setFrom("contact.chandu@coding.com");
        ...
        try{
            ...
            log.info("INFO - Email successfully sent to %s with  template %s ",destinationEmail);
    ...
    public void sendOrderConfirmationEmail(String destinationEmail, String customerName, BigDecimal amount,
                                           String orderReference, List<Product> productList) throws MessagingException {
        MimeMessage mimeMessage = mailSender.createMimeMessage();
        ...
        try{
            //process template Engine using SpringTemplateEngine
            String htmlTemplate = templateEngine.process(templateName,context);
            messageHelper.setText(htmlTemplate,true);
            messageHelper.setTo(destinationEmail);
            // sending message  using java mail sender
            mailSender.send(mimeMessage);
            log.info("INFO - Email successfully sent to %s with  template %s ",destinationEmail);
